...
{
	public class PoisonCharge : ModProjectile
	{
		public override string Texture => "GMR/Projectiles/Melee/SwordSlash2";

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Poison Break");
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			int dustId = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 63, Projectile.velocity.X * 0.5f,
	            Projectile.velocity.Y * 0.2f, 60, Color.Orange, 2f);
		...
		public override void Kill(int timeLeft)
		{
			if (Projectile.alpha < 200)
			{
				for (int i = 0; i < 30; i++)
				{
					var d = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.SilverFlame, newColor: new Color(255, 155, 55, 0), Scale: 1f);
					d.velocity *= 0.4f;
		...
		public override Color? GetAlpha(Color lightColor) => new Color(255, 155, 55, 0);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (int i = 0; i < trailLength; i++)
			{
				...
				Main.EntitySpriteDraw(texture, Projectile.oldPos[i] + drawOffset, frame, new Color(255, 155, 55, 0) * progress * opacity, Projectile.oldRot[i], origin, Projectile.scale, spriteEffects, 0);
			}
			...
			var swishColor = new Color(255, 155, 55, 0) * opacity;
