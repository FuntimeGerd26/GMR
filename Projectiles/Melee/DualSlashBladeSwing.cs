...
namespace GMR.Projectiles.Melee
{
	public class DualSlashBladeSwing : ModProjectile
	{
		...
		{
			DisplayName.SetDefault("Dual Slash Blade");
		}
		...
		public override void AI()
		{
			...
			if (player.dead || player.ItemAnimationActive && player.HeldItem.type != ModContent.ItemType<Items.Weapons.Melee.DualSlashBlade>()
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Color color26 = new Color(55, 145, 55, 125) * opacity;
			...
			Main.EntitySpriteDraw(texture2D13, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), new Microsoft.Xna.Framework.Rectangle?(rectangle), color26,
				Projectile.rotation, origin2, Projectile.scale * 2.15f, spriteEffects, 0);
			Main.EntitySpriteDraw(ModContent.Request<Texture2D>($"GMR/Projectiles/Melee/SwordSlashLight", AssetRequestMode.ImmediateLoad).Value, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY),
				new Microsoft.Xna.Framework.Rectangle?(rectangle), new Color(155, 245, 155, 125) * opacity, Projectile.rotation, origin2, Projectile.scale * 2.15f, spriteEffects, 0);


			Main.EntitySpriteDraw(texture2D13, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), new Microsoft.Xna.Framework.Rectangle?(rectangle), new Color(55, 55, 145, 125) * opacity,
				Projectile.rotation, origin2, Projectile.scale * 1.5f, spriteEffects, 0);
			Main.EntitySpriteDraw(ModContent.Request<Texture2D>($"GMR/Projectiles/Melee/SwordSlashLight", AssetRequestMode.ImmediateLoad).Value, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY),
				new Microsoft.Xna.Framework.Rectangle?(rectangle), new Color(155, 155, 245, 125) * opacity, Projectile.rotation, origin2, Projectile.scale * 1.5f, spriteEffects, 0);
			return false;
		}
	}
}