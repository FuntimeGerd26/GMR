...
{
    public class SilentKatana : HeldSlashingSwordProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (AnimProgress > 0.3f && AnimProgress < 0.6f)
            {
                ...
                for (int i = 0; i < amt; i++)
                {
                    ...
                    var d = Dust.NewDustPerfect(Main.player[Projectile.owner].Center + AngleVector * Main.rand.NextFloat(10f, 70f * Projectile.scale), 54, velocity, newColor: Color.White.UseA(0));
        ...
        public override float GetScale(float progress)
        {
            ...
            {
                return scale + 0.35f * (float)Math.Pow(Math.Sin((progress - 0.1f) / 0.9f * MathHelper.Pi), 2f);
            }
            return scale;
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Vector2 Position = new Vector2(target.Center.X + Main.rand.NextFloat(-100f, 100f), target.Center.Y + Main.rand.NextFloat(-100f, -50f));
            Vector2 Velocity = (target.Center - Position);
            Velocity.Normalize();
            Velocity *= 8f;

            int p = Projectile.NewProjectile(Main.player[Projectile.owner].GetSource_HeldItem(), Position, Velocity, 729, (int)(Projectile.damage * 0.75f), Projectile.knockBack / 4f, Projectile.owner);
            Main.projectile[p].DamageType = DamageClass.Melee;

            int dustId = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 54, Projectile.velocity.X * 0.5f,
                Projectile.velocity.Y * 0.5f, 54, Color.White, 2f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var glowColor = new Color(155, 155, 155, 0);
            ...
            if (intensity > 0f)
            {
                ...
                var shineColor = new Color(255, 255, 255, 100) * intensity * intensity * Projectile.Opacity;
