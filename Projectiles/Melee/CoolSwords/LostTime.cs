...
{
    public class LostTime : CoolSwordBase
    {
        ...
        public override void UpdateSwing(float progress, float interpolatedSwingProgress)
        {
            ...
            if (progress == 0.5f && Main.myPlayer == Projectile.owner)
            {
                ...
                {
                    Vector2 perturbedSpeed = velocity.RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (numberProjectiles - 1))) * 18f;
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center, perturbedSpeed, ModContent.ProjectileType<Projectiles.Melee.LostTimeBlade>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack / 2f, Main.myPlayer);
                }
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 18f, ModContent.ProjectileType<Projectiles.Melee.LostTimeSlash>(), Projectile.damage, Projectile.knockBack / 2f, Main.myPlayer);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var drawColor = Projectile.GetAlpha(new Color(255, 125, 255, 25)) * Projectile.Opacity;
            ...
            if (flip)
            {
                Main.instance.LoadItem(ModContent.ItemType<Items.Weapons.Melee.Swords.LostTime>());
                texture = TextureAssets.Item[ModContent.ItemType<Items.Weapons.Melee.Swords.LostTime>()].Value;
            }
            var origin = new Vector2(0f, texture.Height);

            foreach (var v in GerdHelper.CircularVector(4, Main.GlobalTimeWrappedHourly))
            {
                Main.EntitySpriteDraw(texture, drawCoords + v * 2f * Projectile.scale, null, new Color(255, 105, 105, 0) * Projectile.Opacity, Projectile.rotation, origin, Projectile.scale, effects, 0);
            }
            ...
            if (AnimProgress > 0.45f && AnimProgress < 0.65f)
            {
                ...
                var shineColor = new Color(255, 105, 105, 0) * intensity * intensity * Projectile.Opacity;
            ...
            if (AnimProgress > 0.35f && AnimProgress < 0.75f)
            {
                ...
                var swishColor = new Color(205, 255, 105, 25) * intensity * intensity * Projectile.Opacity * 0.5f;
