...
{
    public class ShadowSword : CoolSwordBase
    {
        ...
        public override void UpdateSwing(float progress, float interpolatedSwingProgress)
        {
            ...
            if (progress == 0.5f && Main.myPlayer == Projectile.owner)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center,
                    velocity,
                    ModContent.ProjectileType<Projectiles.Melee.ShadowSwing>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack / 4f, Projectile.owner);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var drawColor = Color.Black * Projectile.Opacity;
            ...
            if (flip)
            {
                Main.instance.LoadItem(ModContent.ItemType<Items.Weapons.Melee.Swords.ShadowSword>());
                texture = TextureAssets.Item[ModContent.ItemType<Items.Weapons.Melee.Swords.ShadowSword>()].Value;
            }
            var origin = new Vector2(0f, texture.Height);

            foreach (var v in GerdHelper.CircularVector(4, Main.GlobalTimeWrappedHourly))
            {
                Main.EntitySpriteDraw(texture, drawCoords + v * 2f * Projectile.scale, null, Color.Black * Projectile.Opacity, Projectile.rotation, origin, Projectile.scale, effects, 0);
            }
            ...
            if (AnimProgress > 0.45f && AnimProgress < 0.65f)
            {
                ...
                var shineColor = Color.Black * intensity * intensity * Projectile.Opacity;
            ...
            if (AnimProgress > 0.35f && AnimProgress < 0.75f)
            {
                ...
                var swishColor = Color.Black * intensity * intensity * Projectile.Opacity * 0.5f;
