...
{
    public class TrueBraycoeSword : CoolSwordBase
    {
        ...
        public override void UpdateSwing(float progress, float interpolatedSwingProgress)
        {
            ...
            {
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, AngleVector * Projectile.velocity.Length() * 24f, ModContent.ProjectileType<Projectiles.Melee.BraycoeTrueSlash>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack, Projectile.owner);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<BraycoeExplosion>()] < 1)
            Projectile.NewProjectile(player.GetSource_FromThis(), Projectile.Center, AngleVector * Projectile.velocity.Length() * 18f, ModContent.ProjectileType<BraycoeExplosion>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var drawColor = new Color(Main.DiscoR, Main.DiscoG, Main.DiscoB) * Projectile.Opacity;
            ...
            foreach (var v in GerdHelper.CircularVector(4, Main.GlobalTimeWrappedHourly))
            {
                ...
                Main.EntitySpriteDraw(ModContent.Request<Texture2D>($"GMR/Items/Weapons/Melee/TrueBraycoeSword_Glow", AssetRequestMode.ImmediateLoad).Value,
                    drawCoords + v * 2f * Projectile.scale, null, drawColor, Projectile.rotation, origin, Projectile.scale, effects, 0);
            }
    ...
    public class BraycoeExplosion : ModProjectile
    {
        public float colorProgress;
        ...
        public static void ExplosionEffects(int player, Vector2 location, float colorProgress, float scale)
        {
            ...
            for (int i = 0; i < amt; i++)
            {
                ...
                var d = Dust.NewDustPerfect(location + v, DustID.SilverFlame, v / 2.5f, 0, GerdHelper.GetRainbowColor(player, colorProgress + Main.rand.NextFloat(-0.2f, 0.2f)).UseA(0) * Main.rand.NextFloat(0.6f, 1.1f) * scale,
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                var d = Dust.NewDustPerfect(position, DustID.SilverFlame, newColor: GerdHelper.GetRainbowColor(Projectile, Main.GlobalTimeWrappedHourly).UseA(0), Scale: Main.rand.NextFloat(1.5f, 2f));
                d.velocity = r * i / 4f * (Main.rand.NextBool() ? -1f : 1f);
                d.noGravity = true;
            }
            Projectile.NewProjectile(Projectile.GetSource_Death(), position, new Vector2(Projectile.direction, 0f), ModContent.ProjectileType<BraycoeExplosion>(), Projectile.damage / 2, Projectile.knockBack, Projectile.owner);
        }
