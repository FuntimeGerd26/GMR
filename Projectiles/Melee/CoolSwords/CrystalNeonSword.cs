...
namespace GMR.Projectiles.Melee.CoolSwords
{
    public class CrystalNeonSword : CoolSwordBase
    {
        ...
        public override void UpdateSwing(float progress, float interpolatedSwingProgress)
        {
            ...
            if (progress == 0.5f && Main.myPlayer == Projectile.owner)
            {
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, AngleVector * Projectile.velocity.Length() * 9f,
                        ModContent.ProjectileType<Projectiles.Melee.CrystalNeonThrow>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack / 4f, Projectile.owner);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (flip)
            {
                Main.instance.LoadItem(ModContent.ItemType<Items.Weapons.Melee.CrystalNeonSword>());
                texture = TextureAssets.Item[ModContent.ItemType<Items.Weapons.Melee.CrystalNeonSword>()].Value;
            }
            var origin = new Vector2(0f, texture.Height);

            foreach (var v in GerdHelper.CircularVector(4, Main.GlobalTimeWrappedHourly))
            {
                Main.EntitySpriteDraw(texture, drawCoords + v * 2f * Projectile.scale, null, new Color(125, 25, 25, 0) * Projectile.Opacity, Projectile.rotation, origin, Projectile.scale, effects, 0);
            }
            ...
            if (AnimProgress > 0.45f && AnimProgress < 0.65f)
            {
                ...
                var shineColor = new Color(200, 80, 125, 0) * intensity * intensity * Projectile.Opacity;
            ...
            if (AnimProgress > 0.35f && AnimProgress < 0.75f)
            {
                ...
                var swishColor = new Color(255, 110, 185, 125) * intensity * intensity * Projectile.Opacity * 0.5f;
