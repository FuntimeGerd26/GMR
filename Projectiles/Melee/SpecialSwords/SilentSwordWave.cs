...
{
    public class SilentSwordWave : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), new Vector2(target.Center.X + Main.rand.Next(-target.width / 2, target.width / 2),
                target.Center.Y + Main.rand.Next(-target.height / 2, target.height / 2)), Vector2.Zero, ModContent.ProjectileType<SilentHitSpark>(), 0, 0f, Projectile.owner);
            SoundEngine.PlaySound(GMR.GetSounds("Items/Melee/SwordImpact", 3, 0.2f, 0.35f, 0.35f), Projectile.Center);

            Player player = Main.player[Projectile.owner];
            target.AddBuff(ModContent.BuffType<Buffs.Debuffs.SilentWeakening>(), 300);
            player.GetModPlayer<GerdPlayer>().ShakeScreen(3, 0.25f);
        ...
        {
            Lighting.AddLight(Projectile.Center, new Vector3(0.85f, 0.5f, 1f));
            ...
            float num7 = 0.6f;
            float num8 = 0.8f; // Increases the hitbox and visuals
            ...
            {
                Dust dust7 = Dust.NewDustPerfect(Projectile.Center + num10.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), 54, vector3 * 1f);
                dust7.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
                dust7.noGravity = true;
                Dust dust8 = Dust.NewDustPerfect(Projectile.Center + num10.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), 54, vector3 * 1f);
                dust8.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
                dust8.noGravity = true;
            }
            if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity)
            {
                var dust9 = Dust.NewDustPerfect(vector2, 54, vector3 * 1f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            var asset2 = GMR.Instance.Assets.Request<Texture2D>("Projectiles/Melee/SwordSlash2_Light", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
            Rectangle rectangle = asset.Frame(1, 4);
            Vector2 origin = asset.Size() / 2f;
            float num = Projectile.scale * 1.1f;
            SpriteEffects effects = (SpriteEffects)(!(Projectile.ai[0] >= 0f) ? 0 : 2);
            ...
            Color color = new Color(50, 50, 50, 255); // Back Swing
            Main.spriteBatch.Draw(asset.Value, vector, null, color * fromValue * num3, Projectile.rotation + Projectile.ai[0] * ((float)Math.PI / 4f) * -1f * (1f - num2), origin, num, effects, 0f);
            Color color2 = new Color(155, 155, 155, 125); // Front Swing
            // Color color3 = new Color(255, 255, 200, 80);
            Color color4 = new Color(255, 255, 255) * num3 * 0.5f; // Lines
            ...
            Main.spriteBatch.Draw(asset.Value, vector, null, color5 * 0.15f, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, num, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, null, color2 * fromValue * num3 * 0.3f, Projectile.rotation, origin, num, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, null, color2 * fromValue * num3 * 0.5f, Projectile.rotation, origin, num * num4, effects, 0f);
            Main.spriteBatch.Draw(asset2, vector, null, color4 * 0.6f * num3, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, num, effects, 0f);
            Main.spriteBatch.Draw(asset2, vector, null, color4 * 0.5f * num3, Projectile.rotation + Projectile.ai[0] * -0.05f, origin, num * 0.8f, effects, 0f);
            Main.spriteBatch.Draw(asset2, vector, null, color4 * 0.4f * num3, Projectile.rotation + Projectile.ai[0] * -0.1f, origin, num * 0.6f, effects, 0f);
            for (float num5 = 0f; num5 < 8f; num5 += 1f)
            {
                ...
                float num7 = num5 / 9f;
                DrawPrettyStarSparkle(Projectile.Opacity, 0, drawpos, new Color(85, 185, 255, 0) * num3 * num7, color2, num2, 0f, 0.5f, 0.5f, 1f, num6, new Vector2(0f, Utils.Remap(num2, 0f, 1f, 3f, 0f)) * num, Vector2.One * num);
            }
            ...
            DrawPrettyStarSparkle(Projectile.Opacity, 0, drawpos2, new Color(85, 185, 255, 0) * num3 * 0.5f, color2, num2, 0f, 0.5f, 0.5f, 1f, 0f, new Vector2(2f, Utils.Remap(num2, 0f, 1f, 4f, 1f)) * num, Vector2.One * num);
            return false;
        }
    ...
    public class SilentHitSpark : ModProjectile
    {
        public override string Texture => "GMR/Assets/Images/Star08";
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color26 = new Color(155, 155, 155, 125) * opacity;
            if (Scaling > 2f)
                Scaling += 0.025f;

            Main.EntitySpriteDraw(texture2D13, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null,
                color26, MathHelper.ToRadians(90f), origin2, (Projectile.scale * (1f - Scaling)) * 0.135f, SpriteEffects.None, 0);
            Main.EntitySpriteDraw(texture2D13, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null,
                color26, 0f, origin2, (Projectile.scale * (1f - Scaling)) * 0.1f, SpriteEffects.None, 0);
            return false;
        }
    }
}