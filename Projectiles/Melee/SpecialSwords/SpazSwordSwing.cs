...
{
	public class SpazSword : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Vector2 projPos = new Vector2(target.Center.X + Main.rand.Next(-200, 200), target.Center.Y + Main.rand.Next(-600, -300));
            Vector2 velocityToTarget = ((new Vector2(target.Center.X, target.Center.Y)) - projPos) * 0.05f;
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), projPos, velocityToTarget, ModContent.ProjectileType<Projectiles.Melee.SpazEye>(), (int)(damageDone * 0.5), Projectile.knockBack / 4f, Projectile.owner);
            // Visuals
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), new Vector2(target.Center.X + Main.rand.Next(-target.width / 2, target.width / 2),
                target.Center.Y + Main.rand.Next(-target.height / 2, target.height / 2)), Vector2.Zero, ModContent.ProjectileType<SpazHitSpark>(), 0, 0f, Projectile.owner);

            // Projectile Spawn Dust for visuals
            for (int i = 0; i < 10; i++)
            {
                Dust dustId2 = Dust.NewDustDirect(projPos, 10, 5, 61, velocityToTarget.X * 2.5f, velocityToTarget.Y * 0.75f, 60, default(Color), 1.5f);
                dustId2.noGravity = true;

                Dust dustId3 = Dust.NewDustDirect(projPos, 10, 5, 61, -velocityToTarget.X * 2.5f, velocityToTarget.Y * 0.75f, 60, default(Color), 1.5f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color = new Color(0, 200, 0, 255); // Back Swing
            ...
            Color color2 = new Color(125, 185, 85, 200); // Front Swing
    ...
    public class SpazHitSpark : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color26 = new Color(155, 255, 125, 85) * opacity;
            Scaling += -0.025f;
