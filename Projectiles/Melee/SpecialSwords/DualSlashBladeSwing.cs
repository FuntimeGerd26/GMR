...
{
	public class DualSlashBladeSwing : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Vector2 projPos = new Vector2(target.Center.X + Main.rand.Next(-20, 20), target.Center.Y + Main.rand.Next(-20, 20));
            Vector2 velocityToTarget = ((new Vector2(target.Center.X, target.Center.Y)) - projPos) * 0.15f;
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), projPos, velocityToTarget, ModContent.ProjectileType<Projectiles.Melee.DualSlashBladeCut>(),
                Projectile.damage / 4, Projectile.knockBack / 4f, Projectile.owner);

            // Visuals
            Vector2 hitSparkPos = new Vector2(target.Center.X + Main.rand.Next(-target.width / 2, target.width / 2), target.Center.Y + Main.rand.Next(-target.height / 2, target.height / 2));
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), hitSparkPos, Vector2.Zero, ModContent.ProjectileType<SpazHitSpark>(), 0, 0f, Projectile.owner);
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), hitSparkPos, Vector2.Zero, ModContent.ProjectileType<SlashCutterHitSpark>(), 0, 0f, Projectile.owner);
        ...
        public override void AI()
        {
            ...
            float num7 = 0.6f;
            float num8 = 1.4f; // Increases the hitbox and visuals
            ...
            {
                Dust dust7 = Dust.NewDustPerfect(Projectile.Center + num10.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), 61, vector3 * 1f);
                dust7.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
                dust7.noGravity = true;
                Dust dust8 = Dust.NewDustPerfect(Projectile.Center + num10.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), 61, vector3 * 1f);
                dust8.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
                dust8.noGravity = true;
            }
            if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity)
            {
                var dust9 = Dust.NewDustPerfect(vector2, 61, vector3 * 1f);
                dust9.noGravity = true;
                var dust10 = Dust.NewDustPerfect(vector2, 61, vector3 * 1f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color = new Color(185, 185, 25); // Back Swing
            ...
            Color color2 = new Color(0, 185, 125, 125); // Front Swing
            // Color color3 = new Color(255, 255, 200, 80);
            Color color4 = new Color(255, 255, 200) * num3 * 0.5f; // Lines
            ...
            float num8 = Projectile.scale * 0.8f;
            Color color0 = new Color(25, 55, 185, 80);
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)rectangle, color0 * fromValue * num3, Projectile.rotation + Projectile.ai[0] * ((float)Math.PI / 4f) * -1f * (1f - num2), origin, num8, effects, 0f);
            Color color7 = new Color(85, 85, 255, 80);
            //Color color3 = new(227, 182, 245, 80);
            Color color8 = new Color(225, 225, 255) * num3 * 0.5f;
            color8.A = (byte)(float)(int)(color4.A * (1f - fromValue));
            Color color9 = color8 * fromValue * 0.5f;
            color9.G = (byte)(color9.G * fromValue);
            color9.B = (byte)(color9.R * (0.25f + fromValue * 0.75f));
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)rectangle, color9 * 0.15f, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, num8, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)rectangle, color7 * fromValue * num3 * 0.3f, Projectile.rotation, origin, num8, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)rectangle, color7 * fromValue * num3 * 0.5f, Projectile.rotation, origin, num8 * num4, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)asset.Frame(1, 4, 0, 3), color8 * 0.6f * num3, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, num8, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)asset.Frame(1, 4, 0, 3), color8 * 0.5f * num3, Projectile.rotation + Projectile.ai[0] * -0.05f, origin, num8 * 0.8f, effects, 0f);
            Main.spriteBatch.Draw(asset.Value, vector, (Rectangle?)asset.Frame(1, 4, 0, 3), color8 * 0.4f * num3, Projectile.rotation + Projectile.ai[0] * -0.1f, origin, num8 * 0.6f, effects, 0f);
            for (float num5 = 0f; num5 < 8f; num5 += 1f)
            {
                ...
                float num7 = num5 / 9f;
                DrawPrettyStarSparkle(Projectile.Opacity, 0, drawpos, new Color(155, 255, 125, 0) * num3 * num7, color2, num2, 0f, 0.5f, 0.5f, 1f, num6, new Vector2(0f, Utils.Remap(num2, 0f, 1f, 3f, 0f)) * num, Vector2.One * num);
            }
            ...
            DrawPrettyStarSparkle(Projectile.Opacity, 0, drawpos2, new Color(155, 255, 125, 0) * num3 * 0.5f, color2, num2, 0f, 0.5f, 0.5f, 1f, 0f, new Vector2(2f, Utils.Remap(num2, 0f, 1f, 4f, 1f)) * num, Vector2.One * num);
            return false;
        }
