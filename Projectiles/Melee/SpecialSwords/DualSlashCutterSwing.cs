...
namespace GMR.Projectiles.Melee.SpecialSwords
{
    public class DualSlashCutterSwing : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (player.HeldItem.type == ItemType<Items.Weapons.Melee.DualSlashCutter>())
            {
                var swordTexture = GMR.Instance.Assets.Request<Texture2D>("Items/Weapons/Melee/DualSlashCutter", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
    ...
    public class SlashCutterHitSpark : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color26 = new Color(155, 155, 255, 85) * opacity;
            Scaling += -0.025f;

            for (int i = 0; i < 3; i++)
            {
                Main.EntitySpriteDraw(texture2D13, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null,
                    color26, MathHelper.ToRadians(90f), origin2, Projectile.scale * (1f + Scaling * i) * 0.35f, SpriteEffects.None, 0);
                Main.EntitySpriteDraw(texture2D13, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null,
