...
namespace GMR.Projectiles.Ranged
{
    public class EqualBullet : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                var target = Projectile.FindTargetWithLineOfSight(700f);
                if (target != -1)
                {
                    Projectile.velocity = Vector2.Lerp(Projectile.velocity, Vector2.Normalize(Main.npc[target].Center - Projectile.Center) * 18f, 0.25f);
                }
            }
            else
            {
