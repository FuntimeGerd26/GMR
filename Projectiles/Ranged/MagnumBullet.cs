...
namespace GMR.Projectiles.Ranged
{
	public class MagnumBullet : ModProjectile
	{
		...
		{
			int dustId = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 64, Projectile.velocity.X * 1.5f,
				Projectile.velocity.Y * 1.5f, 30, Color.White, 2f);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (float i = 0; i < ProjectileID.Sets.TrailCacheLength[Projectile.type]; i += 0.5f)
			{
				...
				Main.EntitySpriteDraw(texture2D13, value4 + Projectile.Size / 2f - Main.screenPosition + new Vector2(0, Projectile.gfxOffY),
					new Microsoft.Xna.Framework.Rectangle?(rectangle), color27, Projectile.rotation, origin2, new Vector2(Projectile.scale, Projectile.scale * 0.25f), spriteEffects, 0);
			}
			return false;
		}
