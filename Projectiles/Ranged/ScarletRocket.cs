...
namespace GMR.Projectiles.Ranged
{
    public class ScarletRocket : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            var target = Projectile.FindTargetWithLineOfSight(300f);
            if (target != -1)
            {
                Projectile.velocity = Vector2.Lerp(Projectile.velocity, Vector2.Normalize(Main.npc[target].Center - Projectile.Center) * 12f, 0.009f);
            }
        }

        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0.75f, ModContent.ProjectileType<Projectiles.Explosion>(), damageDone / 2, Projectile.knockBack, Main.myPlayer);
