...
namespace GMR.Items.Accessories
{
	public class OverlordBoots : ModItem
	{
		...
		{
			DisplayName.SetDefault("Mech-Overlord Boots");
			Tooltip.SetDefault("Increases movement speed by 15%\nAllows dashing in all directions, while dashing gain 10% damage reduction");
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
		{
			...
			player.GetModPlayer<OverlordDashPlayer>().OverlordDash = true;
		}
	}
	public class OverlordDashPlayer : ModPlayer
	{
		...
		public override void PreUpdateMovement()
		{
			...
			if (CanUseDash() && DashDir != -1 && DashDelay == 0)
			{
					...
					case DashDown when Player.velocity.Y < DashVelocity:
						{
							...
							// This adjustment is roughly 1.5x the intended dash velocity
				...
				for (int i = 0; i < numberProjectiles; i++)
				{
					...
					Projectile.NewProjectile(Player.GetSource_FromThis(), Player.Center, perturbedSpeed + Player.velocity, ModContent.ProjectileType<Projectiles.OverlordOrb>(), 100, 3f, Main.myPlayer);
				}
			...
			{ // dash is active
			  ...
			  // Here we take advantage of "player.eocDash" and "player.armorEffectDrawShadowEOCShield" to get the Shield of Cthulhu's afterimage effect
				...
				Player.endurance = 1f - (0.10f * (1f - Player.endurance));
