...
namespace GMR.Items.Weapons.Melee
{
	public class ThousandSunWrath : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.useTime /= 2;
			Item.SetWeaponValues(160, 2.5f, 18);
			Item.width = 138;
			Item.height = 138;
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			for (int i = 0; i < 4; i++)
			{
				int ProjType = ModContent.ProjectileType<Projectiles.Melee.MagmaEdge>();
				Vector2 target = Main.MouseWorld + 200 * Vector2.UnitX + 200 * Vector2.UnitY;
				Projectile.NewProjectile(player.GetSource_FromThis(), target, new Vector2(-38f, -38f), ProjType, Item.damage, 1f, Main.myPlayer);
				Vector2 target2 = Main.MouseWorld + 200 * Vector2.UnitX - 200 * Vector2.UnitY;
				Projectile.NewProjectile(player.GetSource_FromThis(), target2, new Vector2(-38f, 38f), ProjType, Item.damage, 1f, Main.myPlayer);
				Vector2 target3 = Main.MouseWorld - 200 * Vector2.UnitX + 200 * Vector2.UnitY;
				Projectile.NewProjectile(player.GetSource_FromThis(), target3, new Vector2(38f, -38f), ProjType, Item.damage, 1f, Main.myPlayer);
				Vector2 target4 = Main.MouseWorld - 200 * Vector2.UnitX - 200 * Vector2.UnitY;
				Projectile.NewProjectile(player.GetSource_FromThis(), target4, new Vector2(38f, 38f), ProjType, Item.damage, 1f, Main.myPlayer);
		...
		public override void AddRecipes()
		{
			...
			recipe.AddIngredient(null, "TrueBraycoeSword");
